...
const RegStepper = ({ currentStep = 1, totalSteps = 4, stepTitles = ['Account', 'Personal', 'Vehicle', 'Confirm'] }) => {
  return (
    <div className="w-full flex flex-col font-inter relative z-10">
      <div className="flex items-center justify-between mb-10 relative px-4 lg:px-0">
          ...
          return (
            <React.Fragment key={stepNum}>
              <div className="flex flex-col items-center relative z-10 group">
                {/* Visual Circle */}
                <div className={`w-10 h-10 lg:w-11 lg:h-11 rounded-xl flex items-center justify-center text-sm lg:text-[15px] font-extrabold border-2 transition-all duration-500 transform font-manrope ${
                  isActive
                    ? 'border-[#00D4AA] bg-[#00D4AA]/10 text-[#00D4AA] shadow-lg scale-110'
                    : isCompleted
                    ? 'border-[#00D4AA] bg-gradient-to-br from-[#00D4AA] to-[#00A882] text-[#050F1C]'
                    : 'bg-white/5 border-white/10 text-[#4E7A96] group-hover:bg-white/10 transition-all cursor-default'
                }`}>
                  {isCompleted ? '✓' : stepNum}
                </div>
              ...
              {i < totalSteps - 1 && (
                <div className="flex-1 h-[2px] mx-2 lg:mx-4 relative overflow-hidden bg-white/10 rounded-full top-[1px]">
                  ...
                  />
                  {isActive && <div className="absolute inset-0 bg-[#00D4AA]/20 animate-pulse transition-opacity"></div>}
                </div>
              )}
            </React.Fragment>
          );
      ...
      {/* Divider */}
      <div className="h-px bg-white/5 mx-2 mb-12 mt-12 hidden lg:block" />
    </div>
  );
};
